...
describe("startPluginServices", () => {
  ...
  it("grants internal diagnostics to trusted exporters or conversation-authorized services", async () => {
    ...
    const conversationAuthorizedContexts: OpenClawPluginServiceContext[] = [];
    const conversationAuthorizedService = createTrackingService("langfuse-exporter", {
      contexts: conversationAuthorizedContexts,
    });
    await startPluginServices({
      registry: createRegistry([conversationAuthorizedService], "langfuse-exporter", "workspace"),
      config: {
        plugins: {
          entries: {
            "langfuse-exporter": {
