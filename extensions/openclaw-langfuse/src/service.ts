...
import {
  ...
  TRACE_LEDGER_NAMESPACE,
  TRACE_LEDGER_TTL_MS,
  type TraceLedgerRecord,
...
export function createLangfuseService(
  ...
  logger?: PluginLogger,
  pluginRuntime?: Pick<OpenClawPluginApi["runtime"], "events" | "state">,
): LangfuseService {
  serviceLogger = logger ?? null;
  const serviceOwner = Symbol("openclaw-langfuse-service");
  let ownsActiveRuntime = false;
  // Keep correlation available to the in-process transcript callback without
  // serializing Langfuse identifiers into the 7.1 JSONL session artifact.
  const transientMessageMetadata = new WeakMap<
    object,
  ...
  function codexRolloutMayOwnToolSpans(entry: TraceContextEntry): boolean {
    return (
      codexRolloutOwnsToolSpans(entry) || (unsubscribeDiagnostics !== null && isCodexRuntime(entry))
    );
  }
  ...
  const handlers: LangfuseServiceHookHandlers = {
    ...
    beforeAgentRun(event: BeforeAgentRunEvent, ctx: AgentCtx): BeforeAgentRunResult {
      try {
        ...
        // 7.1 exposes the canonical pre-turn history as `messages`; 7.2 also
        // provides the more explicit `priorMessages` alias.
    ...
    llmInput(event: LlmInputEvent, ctx: AgentCtx): void {
      ...
      if (!entry) {
        ...
        // Codex can emit llm_input without before_agent_run. Keep the root trace
        // useful in that path instead of leaving Langfuse input null.
        const projection = normalizeBeforeAgentRunTraceContext({
          prompt: event.prompt,
          systemPrompt: event.systemPrompt,
        ...
        if (
          beginSdkEnqueue(entry, entry.traceId, "trace-create", "llm_input fallback trace update")
        ) {
          entry.trace.update({
            input: projection.rootInput,
            metadata,
      ...
      if (
        runtimeChanged &&
        beginSdkEnqueue(entry, entry.traceId, "trace-create", "llm_input runtime metadata")
      ) {
        entry.trace.update({ metadata: mergeTraceMetadata(entry, runtimePatch) });
      }
    ...
    llmOutput(event: LlmOutputEvent, ctx: AgentCtx): void {
      ...
      if (pendingGen) {
        ...
        const liveAssistantContent =
          lastAssistantContent ?? (entry.storedOutput.trim() ? entry.storedOutput : undefined);
        const output = liveAssistantContent
          ? buildGenerationOutput(liveAssistantContent, redactEnabled)
          : undefined;
        ...
        if (
          entry.storedOutput.trim() &&
          beginSdkEnqueue(entry, entry.traceId, "trace-create", "llm_output trace update")
        ) {
          // Publish the user-visible reply as soon as llm_output fires. agent_end
          // still replaces it with the canonical transcript-backed final output.
          entry.trace.update({
            output: truncatePayload(redactText(entry.storedOutput, redactEnabled)),
          });
        }
        ...
        // flushAt=1 publishes both updates immediately; agentEnd still performs
        // the final delivery barrier after transcript-backed correction.
      } else {
        serviceLogger?.debug?.(
    ...
    // before_message_write: retain correlation only for in-process callbacks
    beforeMessageWrite(
      event: BeforeMessageWriteEvent,
      ctx: { agentId?: string; sessionKey?: string },
    ): BeforeMessageWriteResult | void {
  ...
  return {
    ...
    async start(ctx: OpenClawPluginServiceContext): Promise<void> {
      ...
      if (pluginRuntime?.state?.openSyncKeyedStore) {
        try {
          const ledgerEnv = serviceStateDir
            ? { ...process.env, OPENCLAW_STATE_DIR: serviceStateDir }
            : process.env;
          configureTraceLedgerStore(
            serviceStateDir,
            pluginRuntime.state.openSyncKeyedStore<TraceLedgerRecord>({
          ...
          serviceLogger.warn(
            `Langfuse: plugin-state trace ledger unavailable; durable recovery is disabled for this process — ${String(error)}`,
          );
        }
      }
