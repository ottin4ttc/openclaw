...
describe("Langfuse diagnostic subscription", () => {
  ...
  it("creates follow-up OpenClaw-native generations from generic per-call diagnostics", async () => {
    const { langfuse, trace, generation } = createLangfuseMock();
  ...
  it("reuses a diagnostic root trace identity after its first SDK enqueue is rejected", async () => {
    ...
    const markerPath = path.join(
      ...
      "openmai-u1",
      "sessions",
      "session-root-retry.langfuse-markers.jsonl",
    );
    const markers = fs
      ...
      .filter((marker) => marker.customType === "langfuse-trace-start");
    expect(markers).toHaveLength(1);
    expect(langfuse.trace).toHaveBeenCalledOnce();
    expect(langfuse.trace.mock.calls[0]?.[0].id).toBe(markers[0]?.data?.traceId);
  ...
  it("writes a tool span end marker before retrying its SDK update", async () => {
    ...
    const markerPath = path.join(
      ...
      "openmai-u1",
      "sessions",
      `${base.sessionId}.langfuse-markers.jsonl`,
    );
    const eventsAfterRejection = fs
      .readFileSync(markerPath, "utf8")
      .trim()
      .split("\n")
      .map((line) => JSON.parse(line) as { e?: string });
    expect(eventsAfterRejection.filter((event) => event.e === "span-end")).toHaveLength(1);
    expect(span.update).not.toHaveBeenCalled();

    emitTrustedDiagnosticEvent(terminal);
    await waitForDiagnosticEventsDrained();

    const eventsAfterRetry = fs
      .readFileSync(markerPath, "utf8")
      .trim()
      .split("\n")
      .map((line) => JSON.parse(line) as { e?: string });
    expect(eventsAfterRetry.filter((event) => event.e === "span-end")).toHaveLength(1);
