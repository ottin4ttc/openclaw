...
describe("trace marker correlation", () => {
  it("does not recover a correlated start after a terminal marker", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "openclaw-langfuse-marker-"));
    try {
      expect(
        writeTraceMarker(dir, "agent-1", "session-1", "start", "trace-1", undefined, {
          correlationKey: "run-1",
        }),
      ).toBe(true);
      expect(readOpenTraceMarkerByCorrelation(dir, "agent-1", "session-1", "run-1")).toMatchObject({
        traceId: "trace-1",
        timestamp: expect.any(Number),
      });

      expect(writeTraceMarker(dir, "agent-1", "session-1", "end", "trace-1")).toBe(true);
      expect(
        readOpenTraceMarkerByCorrelation(dir, "agent-1", "session-1", "run-1"),
      ).toBeUndefined();

      expect(
        writeTraceMarker(dir, "agent-1", "session-1", "start", "trace-2", undefined, {
          correlationKey: "run-2",
        }),
      ).toBe(true);
      expect(
        writeTraceRecoveryMarker(
      ...
      expect(
        readOpenTraceMarkerByCorrelation(dir, "agent-1", "session-1", "run-2"),
      ).toBeUndefined();
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
