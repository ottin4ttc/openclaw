...
import { resolveMarkerFilePath, writeTraceRecoveryMarker } from "./session.js";
...
describe("recoverTrace", () => {
  ...
  it("records terminal abandonment for a trace older than the recovery window", () => {
    ...
    expect(markerRows.at(-1)).toMatchObject({
      customType: "langfuse-trace-recovery",
      data: {
        traceId,
        attempt: 0,
        outcome: "abandoned",
        reason: "trace_age_exceeded",
      },
    });
    ...
    expect(
      fs
        ...
        .trim()
        .split("\n"),
    ).toHaveLength(markerRows.length);
  });
  ...
  it("carries persisted recovery attempts and abandons a trace at the retry limit", () => {
    ...
    expect(markerRows.at(-1)).toMatchObject({
      ...
      data: {
        traceId,
        attempt: TRACE_RECOVERY_MAX_ATTEMPTS,
        outcome: "abandoned",
  ...
  it("does not re-add an abandoned sidecar trace from inline metadata in the same scan", () => {
    const stateDir = makeStateDir();
    const agentId = "agent-1";
    ...
    const recoveryRows = fs
      ...
      .filter(
        (row) =>
          row.customType === "langfuse-trace-recovery" &&
          row.data?.traceId === traceId &&
          row.data?.outcome === "abandoned",
      );
