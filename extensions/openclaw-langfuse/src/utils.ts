...
function wholeMessageSuffixFacts(
  ...
  // Trace metadata is an audit/debug view, not a replay envelope. Project each
  // row to the API-shaped fields that explain the prior turn and drop runtime
  // bookkeeping such as usage, provider identity, timestamps, and mirror tags.
  const projectedMessages = messages
    .map(projectPriorConversationMessage)
    .filter((message): message is unknown => message !== undefined);
  const redacted = redactObject(projectedMessages, redactEnabled);
...
function projectPriorConversationMessage(message: unknown): unknown | undefined {
  if (!message || typeof message !== "object" || Array.isArray(message)) {
    return undefined;
  }
  const candidate = projectLangfusePayload(message) as SessionEntry["message"];
  if (isReasoningOnlyAssistantMessage(candidate)) {
    return undefined;
  }
