...
import { resolveMarkerFilePath, writeObservationEvent } from "./session.js";
  ...
  it("recovers live observation identities and transcript payloads after a gateway service restart", async () => {
    ...
    try {
      ...
      expect(recoveryRows).toEqual([
        expect.objectContaining({
          data: { traceId, attempt: 1, outcome: "started" },
        }),
        expect.objectContaining({
          data: { traceId, attempt: 1, outcome: "succeeded" },
        }),
      ]);
  ...
  it("does not enqueue a terminal transcript generation when gen-end persistence fails", async () => {
    ...
    try {
      ...
      vi.spyOn(fs, "appendFileSync").mockImplementation(((file, data, options) => {
        if (String(data).includes('"e":"gen-end"')) {
          throw new Error("gen-end persistence failed");
        }
        return appendFileSync(file, data, options as never);
