...
export async function prepareCodexAttemptConnection({ params, options }: CodexRunAttemptInput) {
  ...
  const codexModelContentCapture = mergeCodexDiagnosticContentCapturePolicies(
  ...
  const rolloutTraceRoot = hasCodexRolloutTraceContentCapture(codexModelContentCapture)
    ? await prepareCodexRolloutTraceRoot(resolveCodexRolloutTraceRootDir(agentDir))
    : undefined;
...
function mergeCodexDiagnosticContentCapturePolicies(
  left: DiagnosticModelContentCapturePolicy,
  right: DiagnosticModelContentCapturePolicy,
): DiagnosticModelContentCapturePolicy {
  const merged = {
    inputMessages: left.inputMessages || right.inputMessages,
    outputMessages: left.outputMessages || right.outputMessages,
    toolInputs: left.toolInputs || right.toolInputs,
    toolOutputs: left.toolOutputs || right.toolOutputs,
    systemPrompt: left.systemPrompt || right.systemPrompt,
    toolDefinitions: left.toolDefinitions || right.toolDefinitions,
  };
  return {
    ...merged,
    anyModelContent:
      merged.inputMessages ||
      merged.outputMessages ||
      merged.systemPrompt ||
      merged.toolDefinitions,
  };
}

function hasCodexRolloutTraceContentCapture(capture: DiagnosticModelContentCapturePolicy): boolean {
  return capture.anyModelContent || capture.toolInputs || capture.toolOutputs;
}
