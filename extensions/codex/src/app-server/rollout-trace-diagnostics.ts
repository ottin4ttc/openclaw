    ...
    params.client.addCloseHandler(() => {
      ...
      for (const bundleDir of closedRegistration.bundleDirs) {
        ...
        if (owners?.size === 0) {
          ...
          void removeIncompleteRolloutTraceBundleIfUnowned(bundleDir).catch(() => undefined);
        }
      }
...
async function cleanupClosedClientRolloutTraceBundles(params: {
  ...
  for (const bundleDir of await listTraceBundleDirs(params.traceRoot)) {
    ...
    // Bind any other live client for this thread before deciding the bundle is unowned.
    await bindRolloutTraceBundleToLiveOwners(params.traceRoot, bundleDir);
    await removeIncompleteRolloutTraceBundleIfUnowned(bundleDir);
  }
}
...
async function removeIncompleteRolloutTraceBundleIfUnowned(bundleDir: string): Promise<void> {
  const resolvedBundleDir = path.resolve(bundleDir);
  if ((activeRolloutTraceBundleOwners.get(resolvedBundleDir)?.size ?? 0) > 0) {
    return;
  }
  const completed = await traceBundleHasEnded(path.join(resolvedBundleDir, "trace.jsonl")).catch(
    () => false,
  );
  if (!completed && (activeRolloutTraceBundleOwners.get(resolvedBundleDir)?.size ?? 0) === 0) {
    await removeRolloutTraceBundle(resolvedBundleDir);
  }
}
  ...
  for (const event of events.toSorted((left, right) => eventSeq(left) - eventSeq(right))) {
    ...
    if (
      ...
      emitTrustedDiagnosticEventWithPrivateData(
        ...
        modelContentPrivateData(completedContent),
      );
      params.emittedEventKeys.add(eventKey);
      emitted += 1;
      continue;
    ...
    if (toolTerminal.status === "completed") {
      emitTrustedDiagnosticEventWithPrivateData(
        { type: "tool.execution.completed", ...commonFields },
        privateData,
      );
    } else {
      emitTrustedDiagnosticEventWithPrivateData(
