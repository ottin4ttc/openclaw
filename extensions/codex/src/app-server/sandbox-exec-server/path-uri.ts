...
/** Formats one POSIX sandbox path as a Codex PathUri. */
export function formatExecServerPathUri(sandboxPath: string): string {
  if (!sandboxPath.startsWith("/")) {
    throw new Error("sandbox path must be an absolute POSIX path.");
  }
  return pathToFileURL(sandboxPath, { windows: false }).href;
}
