...
describe("OpenClaw Codex sandbox exec-server filesystem", () => {
  ...
  it("returns the complete Codex exec-server metadata shape", async () => {
    const sandbox = createSandboxContext({
      stat: async () => ({ type: "file", size: 42, mtimeMs: 7 }),
    });
    const client = createClient();
    await ensureCodexSandboxExecServerEnvironment({
      client: client as never,
    ...
    await expect(
      rpc(socket, "fs/getMetadata", { path: "file:///workspace/AGENTS.md" }),
    ).resolves.toEqual({
      isDirectory: false,
      isFile: true,
      isSymlink: false,
  ...
  it("treats metadata probes above the mounted workspace as missing", async () => {
    const stat = vi.fn(async ({ filePath }: { filePath: string }) => {
      if (filePath === "/.git") {
        throw new Error(
          "Path escapes sandbox root (/host/workspace; container root /workspace): /.git",
        );
      }
      return null;
    });
    const sandbox = createSandboxContext({ stat });
    const client = createClient();
    await ensureCodexSandboxExecServerEnvironment({
      client: client as never,
      sandbox,
    ...
    await expect(rpc(socket, "fs/getMetadata", { path: "file:///.git" })).rejects.toThrow(
      "file not found",
    );
    await expect(rpc(socket, "fs/readFile", { path: "file:///.git" })).rejects.toThrow(
