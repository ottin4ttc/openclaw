...
### Skill locations in native harnesses

Harnesses that render OpenClaw's Skill list should resolve it only after the
effective workspace and sandbox are known. Import
`resolveAgentHarnessSkillsPromptForRun` from
`openclaw/plugin-sdk/agent-harness-runtime` and pass the prepared attempt,
effective workspace, sandbox context, and active agent id. This keeps each
`<location>` path valid inside the execution environment instead of leaking a
host-only path into a containerized turn.

These locations are filesystem paths. A native runtime should read them with
its filesystem or command tools. Do not pass them to a runtime-specific Skill
API that expects package or resource handles, such as Codex `skills.read`.
