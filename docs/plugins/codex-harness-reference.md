...
plugin (currently `@openai/codex` `0.148.0`):
...
| `nativeHookRelay.enabled`                     | `true`                                                 | Controls only the OpenClaw compatibility relay for Codex-native tool and finalization hooks. Set `false` to avoid injecting OpenClaw relay commands; Codex-owned user and plugin hooks remain enabled. Restart the Gateway after changing this setting.                                                                                                                                         |
...
<Note>
The current bundled harness is `@openai/codex` `0.148.0`. A `model/list` probe
